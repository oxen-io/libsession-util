...
///
/// Inputs:
/// - `curve25519_pk_out` -- [out] pointer to a buffer of 32 bytes where the public key will be written.
/// - `curve25519_sk_out` -- [out] pointer to a buffer of 32 bytes where the private key will be written.
///
/// Outputs:
/// - `bool` -- True if the seed was successfully retrieved, false if failed.
LIBSESSION_EXPORT bool session_curve25519_key_pair(
    unsigned char* curve25519_pk_out,
    unsigned char* curve25519_sk_out);
...
/// - `curve25519_sk_out` -- [out] pointer to a buffer of 32 bytes where the secret key will be written.
///
/// Outputs:
