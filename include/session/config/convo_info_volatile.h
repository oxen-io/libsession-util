...
///
/// Inputs:
/// - `ed25519_secretkey` -- [out] must be the 32-byte secret key seed value.  (You can also pass the
...
///
/// - `dump` -- [in, optional] if non-NULL this restores the state from the dumped byte string produced by a past
...
///
/// Outputs:
/// - `int` --Returns 0 on success; returns a non-zero error code and write the exception message as a
...
///
/// Same as the above convo_info_volatile_get_1to1 except that when the conversation does not exist, this sets all the convo
...
///
/// Gets a community convo info, but if the community does not exist will set all the fileds to defaults and load it.
/// `base_url` and `room` are null-terminated c strings; pubkey is
